...
#define MEMASSERT(p,s) if(!p){printf("FATAL: ACSM-No Memory: %s!\n",s);}
...
*
*/
void acsmSetVerbose2(bool is_verbose)
{
     s_verbose = is_verbose? TRUE : FALSE;
}
    ...
    if (p != NULL)
    {
        switch (type)
        {
            ...
            case ACSM2_MEMORY_TYPE__NONE:
                break;
            default:
                printf("FATAL: %s(%d) Invalid memory type\n", __FILE__, __LINE__);
                break;
        }
